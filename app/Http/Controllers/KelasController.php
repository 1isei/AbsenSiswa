...
class KelasController extends Controller
{
    public function index(){
        ...
        return view('Admin/kelas', compact('    ', 'resource'),['resource'=>$resource]);
    }
    public function create(Request $request){
        $check = Kelas::where(['tingkat_kelas' => $request->tingkat_kelas, 'id_jurusan' => $request->id_jurusan, 'nama_kelas' => $request->nama_kelas, 'tahun_masuk' => $request->tahun_masuk, 'tahun_keluar' => $request->tahun_keluar])->get();
        if($check->count()>0){
